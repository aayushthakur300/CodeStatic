...
from fpdf import FPDF, XPos, YPos # Requires 'pip install fpdf2'
...
# --------------------------------------------------------------------
# 🔹 DATABASE SETUP (SQLITE) - THREAD SAFE FIX
# --------------------------------------------------------------------
# Define base directory ensuring it works on Render/Local
BASE_DIR = Path(__file__).resolve().parent
DB_PATH = BASE_DIR / "app.db"

# Connect to SQLite
# check_same_thread=False is required for Flask, but we must NOT share cursors.
conn = sqlite3.connect(str(DB_PATH), check_same_thread=False)

# This allows us to access columns by name (row['id']) instead of index (row[0])
conn.row_factory = sqlite3.Row 

# Initialize Tables (Use a temporary cursor for setup)
def init_db():
    try:
        cur = conn.cursor()
        cur.executescript("""
...
class CodeReportPDF(FPDF):
    ...
    def code_block(self, code_text):
        ...
        self.multi_cell(0, 5, code_text, border=0, align='L', fill=False, dry_run=True)
...
# --- ROUTES UPDATED FOR LANDING PAGE + TOOL ---
...
def logic_probe():
    # Renders the Assessment Tool (The file you renamed to tool.html)
    return render_template('tool.html')

# --------------------------------------------------------------------
# 🔹 PROJECT & DATABASE ROUTES (FIXED FOR THREADING)
# --------------------------------------------------------------------
...
def save_code():
    try:
        ...
        cur = conn.cursor()
        cur.execute("INSERT INTO code_history (code, language) VALUES (?, ?)", (code, language))
        conn.commit()
        return jsonify({"status": "success", "id": cur.lastrowid})
    except Exception as e:
        return jsonify({"status": "error", "error": str(e)}), 500

@app.route('/load-last-code', methods=['GET'])
def load_last_code():
    try:
        cur = conn.cursor()
...
def load_chat():
    try:
        ...
        rows = cur.fetchall()
        return jsonify({"status": "success", "chat": [dict(r) for r in rows]})
    except Exception as e:
        return jsonify({"status": "error", "error": str(e)}), 500
...
def get_projects():
    try:
        ...
        rows = cur.fetchall()
        return jsonify({"status": "success", "projects": [dict(r) for r in rows]})
    except Exception as e:
        # This catches the 500 error you saw
        return jsonify({"status": "error", "error": str(e)}), 500

@app.route('/favorite-project', methods=['POST'])
def favorite_project():
    try:
        data = request.json
        p_id = data.get('id')
        fav = data.get('fav') # Expect boolean or 0/1

        val = 1 if fav else 0
        cur = conn.cursor()
        cur.execute("UPDATE projects SET is_favorite = ? WHERE id = ?", (val, p_id))
        conn.commit()
...
def generate_pdf():
    try:
        ...
        if not report_data or not report_data.get('final_code'):
            return jsonify({"status": "error", "message": "Missing required report data (run assessment first)."}), 400
        ...
        return jsonify({"status": "error", "message": f"PDF Generation Failed: {str(e)}. Please check that 'fpdf2' is installed."}), 500


# --- AI CHAT ASSISTANT ROUTE (With DB Saving) ---
